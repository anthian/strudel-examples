...
bassline: note("[eb1, eb2]!16 [f2, f1]!16 [g2, g1]!16 [f2, f1]!8 [bb2, bb1]!8")
  ...
  .slow(8)
  .postgain(2)
  .lpf(slider(1561.4,300,2000))
  .room(0.6)
  .postgain(pick(gainnn, gooo))
  ._punchcard({height:200, width:1670}).color("lightgreen")
...
main_arp: note(pick(arpeggiator, "<0 1 2 3>".slow(2)))
  ...
  .room(0.6)
  .lpenv(slider(4.70325, 1.25, 6))
  .postgain(pick(gainnn, gooo))
  ._punchcard({height:200, width:1670}).color("yellow")

drums: stack(
  s("tech:5").postgain(6).pcurve(2).pdec(1).struct(pick(Structures, gooo))
)._punchcard({height:50, width:1670}).color("cyan")